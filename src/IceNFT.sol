...
contract IceNFT is ERC721, AccessControl, ReentrancyGuard {
    ...
    constructor(
        ...
        require(
            ...
            "IceNFT: fund manager is the zero address"
        );
    ...
    event SetRelationship(
        ...
        address indexed upUpline
    );
    event Burn(uint256 tokenId);
    event Mint(
        address indexed to,
        NFTCardType indexed cardType,
        uint256 tokenId,
        string currency,
        uint256 price
    );
    ...
    function setWhitelistMintCardType(
        NFTCardType _cardType
    ) external onlyOperator {
    ...
    function flipWhitelistMintState() external onlyOperator {
        require(
            whitelistMintMerkleRoot != bytes32(0),
            "Merkle root haven't set"
        );
    ...
    function whitelistMint(bytes32[] calldata merkleProof) public payable {
        ...
        if (whitelistMintCardType == NFTCardType.LIFETIME) {
            require(
                lifetimeCardMaxSupply >= lifetimeCardCurrentSupply,
                "Lifetime card sold out"
            );
        }
    ...
    function mintWithWeth(
        ...
        IERC20(wethToken).safeTransferFrom(msg.sender, address(this), amount);

        _tokenIds.increment();
        uint256 tokenId = _tokenIds.current();
        transferFlag = true;
        _safeMint(msg.sender, tokenId);
        emit Mint(msg.sender, cardType, tokenId, "weth", ethPrice[cardType]);
        cardTypeOfTokenId[tokenId] = cardType;
        transferFlag = false;
        if (cardType != NFTCardType.LIFETIME) {
            lifetimeCardCurrentSupply++;
    ...
    function _allocateEthReward(uint256 amount) internal {
        (address upline, address upUpline) = getReferralRelationship(
            msg.sender
        );
        ...
        if (saveFlag) {
            (bool success, ) = payable(fundManager).call{
                value: msg.value - remains
            }("");
    ...
    function setReferralRelationship(
        address user,
        address referredBy
    ) internal {
        ...
        require(
            ...
            "Referral: referredBy is zero address"
        );

        // check if user's upline is already set
        require(
            usersUpline[user] == address(0),
            "Referral: user's upline is already set"
        );
    ...
    function setRewardRate(uint8 upline, uint8 upUpline) external onlyOperator {
        ...
        require(
            upUpline <= 100,
            "Referral: upline's upline reward rate is over 100%"
        );
    ...
    function claimUsdReward() external nonReentrant {
        ...
        require(
            IERC20(usdToken).balanceOf(address(this)) >=
                referralRewardUsdAmount[msg.sender],
            "Balance insufficient for reward"
        );
    ...
    function _beforeTokenTransfer(
        ...
        require(
            transferFlag ||
                cardTypeOfTokenId[firstTokenId] == NFTCardType.LIFETIME,
